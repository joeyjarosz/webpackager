...
// expiry.
// 9 days is recommended duration to start requesting new certs to allow for
// ACME server outages. It's 7 days + 2 days renewal grace period. 7 days so
...
func (c *Client) Fetch(chain *certchain.RawChain, now func() time.Time) (newChain *certchain.RawChain, nextRun futureevent.Event, err error) {
	...
	// Implementing it involves larger code change, e.g. you'll probably need to
...
// given expiry.  Note that the whenTheCertExpires should be the expected SXG
